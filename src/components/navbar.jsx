...
const Navbar = () => {
  ...
  const linkClass = (id) =>
    `rounded-full px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.12em] transition-all duration-200 ${
      activeLink === id
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 md:px-6 lg:px-8">
      <div className="mx-auto flex h-[5rem] w-full max-w-[1300px] items-center justify-between rounded-2xl border border-slate-300/70 bg-white/74 px-4 shadow-[0_10px_28px_rgba(15,23,42,0.16)] backdrop-blur-xl dark:border-white/22 dark:bg-slate-950/82 dark:shadow-[0_12px_34px_rgba(2,6,23,0.46)]">
        <div className="flex items-center">
          <img
            src={rootsLogo}
            alt="ROOTS logo"
            className="h-14 w-auto object-contain brightness-110 contrast-150 saturate-[1.7] drop-shadow-[0_0_26px_rgba(52,211,153,0.45)] md:h-16"
          />
        </div>
        ...
        <div className="flex items-center gap-1">
          <div className="hidden items-center gap-1 rounded-full border border-slate-300/70 bg-white/72 p-1 sm:flex dark:border-white/22 dark:bg-slate-900/80">
          ...
          <button
            type="button"
            className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-300/75 text-slate-700 transition-all duration-200 hover:bg-slate-900/10 md:hidden dark:border-white/22 dark:text-slate-100 dark:hover:bg-white/14"
      ...
      {showMenu && (
        <nav className="mx-auto mt-2 flex w-full max-w-[1300px] flex-col gap-2 rounded-2xl border border-slate-300/80 bg-white/92 p-3 text-slate-900 shadow-[0_16px_44px_rgba(15,23,42,0.2)] backdrop-blur-xl dark:border-white/22 dark:bg-slate-950/94 dark:text-white dark:shadow-[0_16px_44px_rgba(2,6,23,0.52)] md:hidden">
          {NAV_LINKS.map((item) => (
            <Link key={item.id} to={item.id} smooth duration={800} offset={NAV_OFFSET}>
